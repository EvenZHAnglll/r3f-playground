...
export class GridMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      uniforms: {
        time: { value: 0 },
        uvScale: { value: 40 },
        dotColor: { value: new THREE.Color("#a4c0f7") },
  ...
  setDotColor(color: string | THREE.Color) {
    this.uniforms.dotColor.value = new THREE.Color(color);
  }

  setSecondaryOpacity(opacity: number) {
    this.uniforms.secondaryOpacity.value = opacity;
  }
